...
func main() {
	...
	services := []Service{
		...
		{
			Name:         "Firewall Engine V5",
			ExePath:      filepath.Join(binDir, "firewall-engine", "firewall-engine.exe"),
			WorkDir:      filepath.Join(binDir, "firewall-engine"),
			RequireAdmin: true,
	...
	fmt.Println("║  Firewall Engine V5   │ ~2-5% (dual-engine enforcement)       ║")
