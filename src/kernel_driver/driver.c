...
// Registry paths
#define SAFEOPS_REGISTRY_PATH      L"\\Registry\\Machine\\System\\CurrentControlSet\\Services\\SafeOps"
...
#define SAFEOPS_LOG_ERROR(format, ...)   SAFEOPS_LOG(LOG_LEVEL_ERROR, "ERROR: " format, __VA_ARGS__)
#define SAFEOPS_LOG_WARNING(format, ...) SAFEOPS_LOG(LOG_LEVEL_WARNING, "WARNING: " format, __VA_ARGS__)
#define SAFEOPS_LOG_INFO(format, ...)    SAFEOPS_LOG(LOG_LEVEL_INFO, "INFO: " format, __VA_ARGS__)
#define SAFEOPS_LOG_DEBUG(format, ...)   SAFEOPS_LOG(LOG_LEVEL_DEBUG, "DEBUG: " format, __VA_ARGS__)
#define SAFEOPS_LOG_VERBOSE(format, ...) SAFEOPS_LOG(LOG_LEVEL_VERBOSE, "VERBOSE: " format, __VA_ARGS__)
...
VOID SafeOpsRollbackInitialization(_In_ PDRIVER_CONTEXT Context, _In_ ULONG CompletedSteps);
...
NTSTATUS SafeOpsPowerCallback(_In_ PVOID CallbackContext, _In_ PVOID Argument1, _In_ PVOID Argument2);
...
NTSTATUS SafeOpsWriteEventLog(_In_ ULONG EventType, _In_ ULONG EventId, _In_ PWSTR Message);
...
)
{
    ...
    SAFEOPS_LOG_INFO("=== SafeOps Driver v%s Starting ===", SAFEOPS_VERSION_STRING);
    ...
    if (!NT_SUCCESS(status)) {
        SAFEOPS_LOG_WARNING("Failed to read registry config: 0x%08X (using defaults)", status);
    ...
    SAFEOPS_LOG_INFO("  Logging: %s", g_DriverConfig.EnableLogging ? "Enabled" : "Disabled");
    SAFEOPS_LOG_INFO("  Deep Inspection: %s", g_DriverConfig.EnableDeepInspection ? "Enabled" : "Disabled");
    ...
    if (!NT_SUCCESS(status)) {
        ...
        SafeOpsWriteEventLog(EVENTLOG_ERROR_TYPE, 1001, L"Failed to create WDF driver object");
    ...
    SafeOpsWriteEventLog(EVENTLOG_INFORMATION_TYPE, 1000, L"SafeOps Driver loaded successfully");
...
)
{
    ...
    SafeOpsWriteEventLog(EVENTLOG_INFORMATION_TYPE, 1002, L"SafeOps Driver unloaded successfully");
 ...
 */
NTSTATUS
SafeOpsInitializeAllComponents(
...
)
{
    ...
    if (!NT_SUCCESS(status)) {
        SAFEOPS_LOG_WARNING("PerformanceInitialize failed: 0x%08X (non-fatal)", status);
        // Non-fatal, continue
    } else {
        step = 4;
...
rollback:
    SAFEOPS_LOG_ERROR("Component initialization failed at step %u, rolling back", step);
