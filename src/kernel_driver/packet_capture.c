 ...
 *   Implements NDIS 6.x filter driver with zero-copy ring buffer, flow tracking,
 *   smart deduplication, and protocol parsing.
 * 
 * FEATURES:
...
static VOID ProcessPacketInternal(PFILTER_MODULE_CONTEXT filter_ctx, PNET_BUFFER_LIST nbl, BOOLEAN is_send);
...
)
{
    ...
    g_DriverContext = ExAllocatePoolWithTag(NonPagedPool, sizeof(DRIVER_CONTEXT), 'SfOp');
    if (!g_DriverContext) {
    ...
    filterChars.Header.Size = NDIS_SIZEOF_FILTER_DRIVER_CHARACTERISTICS_REVISION_2;
    ...
    status = NdisFRegisterFilterDriver(
        DriverObject,
        (NDIS_HANDLE)g_DriverContext,
        &filterChars,
        &g_NdisFilterDriverHandle
    );
    ...
    KeSetTimerEx(&g_DriverContext->rotation_timer, dueTime, LOG_ROTATION_INTERVAL_MS, &g_DriverContext->rotation_dpc);
...
static NTSTATUS
InitializeDriverContext(PDRIVER_CONTEXT ctx)
{
    NTSTATUS status;
    ...
    KeInitializeDpc(&ctx->cleanup_dpc, (PKDEFERRED_ROUTINE)FlowCleanupExpired, ctx);
...
static VOID
CleanupDriverContext(PDRIVER_CONTEXT ctx)
{
    if (!ctx) return;
...
)
{
    ...
    filterCtx = ExAllocatePoolWithTag(NonPagedPool, sizeof(FILTER_MODULE_CONTEXT), 'FmCt');
    if (!filterCtx) {
    ...
    if (AttachParameters->BaseMiniportName) {
        RtlCopyMemory(filterCtx->nic_info.friendly_name, 
                      ...
                      min(AttachParameters->BaseMiniportName->Length, sizeof(filterCtx->nic_info.friendly_name) - 2));
    }
...
{
    PFILTER_MODULE_CONTEXT filterCtx = (PFILTER_MODULE_CONTEXT)FilterModuleContext;
...
{
    NdisFSendNetBufferListsComplete(FilterModuleContext, NetBufferLists, SendCompleteFlags);
}
...
)
{
    ...
    NdisFIndicateReceiveNetBufferLists(
        FilterModuleContext, NetBufferLists, PortNumber,
...
)
{
    ...
    metadata.sequence_number = InterlockedIncrement64((LONGLONG*)&ctx->stats.packets_captured);
    metadata.nic_id = filterCtx->nic_id;
    metadata.direction = is_send ? FLOW_DIRECTION_OUTBOUND : FLOW_DIRECTION_INBOUND;
    ...
    if (ctx->config.mode >= CAPTURE_MODE_PARTIAL_PAYLOAD) {
        status = CapturePayload(nb, payload_buffer, ctx->config.snapshot_length, &payload_len);
        if (NT_SUCCESS(status)) {
    ...
    switch (metadata.ip_protocol) {
        case IP_PROTO_TCP: ctx->stats.tcp_packets++; break;
        case IP_PROTO_UDP: ctx->stats.udp_packets++; break;
        case IP_PROTO_ICMP: ctx->stats.icmp_packets++; break;
        default: ctx->stats.other_packets++; break;
    }
}

VOID
ProcessPacket(
    PFILTER_MODULE_CONTEXT filter_ctx,
    PNET_BUFFER_LIST nbl,
    BOOLEAN is_send
)
{
...
VOID
RingBufferDestroy(PDRIVER_CONTEXT ctx)
{
    if (ctx->ring_buffer_va) {
...
)
{
    ...
    UINT64 alignedSize = (totalSize + CACHE_LINE_SIZE - 1) & ~(CACHE_LINE_SIZE - 1);
    UINT64 writeIdx, nextWriteIdx;
    PUCHAR dataStart = (PUCHAR)ctx->ring_buffer_va + sizeof(RING_BUFFER_HEADER);
    PUCHAR writePtr;

    if (!header) return STATUS_INVALID_DEVICE_STATE;
    if (alignedSize > header->size) return STATUS_BUFFER_TOO_SMALL;
    ...
    do {
        ...
        if (RingBufferIsFull(ctx)) {
            ...
            return STATUS_BUFFER_OVERFLOW;
                                          ...
                                          nextWriteIdx, writeIdx) != (LONGLONG)writeIdx);
...
{
    PRING_BUFFER_HEADER header = ctx->ring_header;
    if (!header || header->size == 0) return 0;
...
)
{
    ...
    if (!nb || !metadata) return STATUS_INVALID_PARAMETER;

    RtlZeroMemory(metadata, sizeof(PACKET_METADATA));

    bufferLength = NET_BUFFER_DATA_LENGTH(nb);
    if (bufferLength < 14) return STATUS_BUFFER_TOO_SMALL;

    buffer = NdisGetDataBuffer(nb, min(bufferLength, 256), NULL, 1, metaBuf, FALSE);
    ...
    if (metadata->ethertype == ETHERTYPE_IPV4 && bufferLength >= offset + 20) {
        ...
        if (metadata->ip_protocol == IP_PROTO_TCP && bufferLength >= offset + 20) {
            ...
            offset += (metadata->tcp_data_offset ? metadata->tcp_data_offset * 4 : 20);
        }
        // Parse UDP
        else if (metadata->ip_protocol == IP_PROTO_UDP && bufferLength >= offset + 8) {
            ParseUdpHeader(buffer + offset, bufferLength - offset, metadata);
            offset += 8;
        }
    }
    // Parse IPv6
    else if (metadata->ethertype == ETHERTYPE_IPV6 && bufferLength >= offset + 40) {
    ...
    if (metadata->dst_port == 80 || metadata->src_port == 80) metadata->app_protocol = APP_PROTO_HTTP;
    else if (metadata->dst_port == 443 || metadata->src_port == 443) metadata->app_protocol = APP_PROTO_HTTPS;
    else if (metadata->dst_port == 53 || metadata->src_port == 53) metadata->app_protocol = APP_PROTO_DNS;
    else if (metadata->dst_port == 22 || metadata->src_port == 22) metadata->app_protocol = APP_PROTO_SSH;

    metadata->payload_offset = (UINT16)offset;
    metadata->packet_length_captured = (UINT16)min(bufferLength, MAX_SNAPSHOT_LENGTH);
...
{
    if (len < 20) return STATUS_BUFFER_TOO_SMALL;

    metadata->src_port = (data[0] << 8) | data[1];
    metadata->dst_port = (data[2] << 8) | data[3];
    metadata->tcp_seq = (data[4] << 24) | (data[5] << 16) | (data[6] << 8) | data[7];
    metadata->tcp_ack = (data[8] << 24) | (data[9] << 16) | (data[10] << 8) | data[11];
...
)
{
    ...
    if (!nb || !buffer || !captured_len) return STATUS_INVALID_PARAMETER;
...
FlowTrackingInitialize(PDRIVER_CONTEXT ctx)
{
    ...
    RtlZeroMemory(ctx->flow_hash_table, ctx->flow_hash_buckets * sizeof(PFLOW_CONTEXT));
    return STATUS_SUCCESS;
}

VOID
FlowTrackingDestroy(PDRIVER_CONTEXT ctx)
{
    if (ctx->flow_hash_table) {
...
FlowLookupOrCreate(PDRIVER_CONTEXT ctx, PFLOW_KEY key)
{
    ...
    while (flow) {
        if (RtlCompareMemory(&flow->key, key, sizeof(FLOW_KEY)) == sizeof(FLOW_KEY)) {
    ...
    if (flow) {
        ...
        flow->flow_id = InterlockedIncrement64((LONGLONG*)&ctx->stats.flows_created);
...
VOID
FlowUpdate(PFLOW_CONTEXT flow, PPACKET_METADATA metadata)
{
...
VOID
FlowCleanupExpired(PDRIVER_CONTEXT ctx)
{
...
DeduplicationInitialize(PDRIVER_CONTEXT ctx)
{
    ...
    RtlZeroMemory(ctx->dedup_hash_table, ctx->dedup_hash_buckets * sizeof(PDEDUP_ENTRY));
    return STATUS_SUCCESS;
}

VOID
DeduplicationDestroy(PDRIVER_CONTEXT ctx)
{
    if (ctx->dedup_hash_table) {
...
BOOLEAN
DeduplicationCheckUnique(PDRIVER_CONTEXT ctx, PPACKET_METADATA metadata, PUINT8 reason)
{
    UINT64 signature;
    UINT32 bucket;
    PDEDUP_ENTRY entry;
    ...
    if (IsCriticalPort(metadata->src_port) || IsCriticalPort(metadata->dst_port)) {
    ...
    if (metadata->ip_protocol == IP_PROTO_TCP &&
        (metadata->tcp_flags.syn || metadata->tcp_flags.fin || metadata->tcp_flags.rst)) {
...
VOID
FilterEngineDestroy(PDRIVER_CONTEXT ctx)
{
    if (ctx->filter_rules) {
...
VOID
NicReleaseId(PDRIVER_CONTEXT ctx, UINT8 nic_id)
{
    KIRQL oldIrql;

    if (nic_id >= MAX_NICS) return;

    KeAcquireSpinLock(&ctx->nic_lock, &oldIrql);
    RtlZeroMemory(&ctx->nics[nic_id], sizeof(NIC_INFO));
    if (ctx->nic_count > 0) ctx->nic_count--;
    KeReleaseSpinLock(&ctx->nic_lock, oldIrql);
}
...
{
    if (nic_id >= MAX_NICS || !info) return STATUS_INVALID_PARAMETER;
...
VOID
UpdateStatistics(PDRIVER_CONTEXT ctx, PPACKET_METADATA metadata)
{
    UNREFERENCED_PARAMETER(ctx);
    UNREFERENCED_PARAMETER(metadata);
...
{
    if (!ctx || !stats) return STATUS_INVALID_PARAMETER;
    ...
    stats->uptime_seconds = (GetSystemTimestamp() - ctx->stats.start_time) / 10000000ULL;
...
VOID
SafeCopyMemory(PVOID dest, PVOID src, SIZE_T len)
{
    __try {
